...
add_test("verb_ask() with no target asks first person in room about all words", "test_ask_gvn_no_comma")
add_test("verb_ask() with no target and matching keyword gets a real answer", "test_ask_gvn_no_comma_matches")
add_test("verb_ask() with no target and no-one in room fails gracefully", "test_ask_gvn_no_comma_no_person")
add_test("verb_ask() with no target picks first person, not second", "test_ask_no_comma_first_person")
add_test("verb_ask() with no target and single word topic", "test_ask_no_comma_one_word")
...
' non-wildcard answer.
Sub test_ask_gvn_no_comma_matches()
...
Sub test_ask_gvn_no_comma_no_person()
  ...
  assert_int_equals(1, result%)
  Const expected$ = "[[red:There is no-one here to talk to.]]" + sys.CRLF$
  assert_string_equals(expected$, con_output$)
End Sub
...
Sub test_ask_no_comma_first_person()
  Local result% = parse_common("ask gramophone")
  assert_int_equals(0, result%)
...
' full subject.
Sub test_ask_no_comma_one_word()
  Local result% = parse_common("ask gramophone")
  assert_int_equals(0, result%)
...
Sub test_ask_gvn_no_target()
  Local result% = parse_common("ask nonexistent thing, gramophone")
  assert_int_equals(0, result%)
...
Sub test_ask_gvn_not_here()
  Local result% = parse_common("ask other suspect, gramophone")
  assert_int_equals(0, result%)
...
Sub test_ask_gvn_not_person()
  Local result% = parse_common("ask green door, gramophone")
  assert_int_equals(0, result%)
...
Sub test_ask_gvn_no_wildcard()
  Local result% = parse_common("ask no wildcard, something else entirely")
  assert_int_equals(0, result%)
...
Sub test_ask_gvn_wildcard_fallback()
  Local result% = parse_common("ask test suspect, something else entirely")
  assert_int_equals(0, result%)
...
Sub test_ask_skips_ineligible()
  Local result% = parse_common("ask test suspect, gramophone")
  assert_int_equals(0, result%)
...
Sub test_ask_gvn_all_blocked()
  Local result% = parse_common("ask test suspect, piano only")
  assert_int_equals(0, result%)
...
Sub test_ask_always_returns_handled()
  ...
  result% = parse_common("ask nonexistent thing, gramophone")
  assert_int_equals(1, verb_ask())

  result% = parse_common("ask test suspect, gramophone")
  assert_int_equals(1, verb_ask())
End Sub

' "ask, topic" - comma is the very first word, so no target was
...
Sub test_ask_gvn_plus_missing()
  Local result% = parse_common("ask test suspect, news")
  assert_int_equals(0, result%)
...
Sub test_ask_gvn_plus_present()
  Local result% = parse_common("ask test suspect, urgent news")
  assert_int_equals(0, result%)
...
Sub test_ask_gvn_minus_absent()
  Local result% = parse_common("ask test suspect, quiet")
  assert_int_equals(0, result%)
...
Sub test_ask_gvn_minus_present()
  Local result% = parse_common("ask test suspect, quiet secret")
  assert_int_equals(0, result%)
